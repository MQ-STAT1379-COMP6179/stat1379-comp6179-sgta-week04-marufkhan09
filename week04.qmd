...
no they are not prop gives the most dominant in that year and count gives the largest number of babies in the year.
...
cholesterol_long <- cholesterol_data |>
  pivot_longer(
    ...
    names_to = "diet",         # New column for diet group
    values_to = "cholesterol"  # New column for cholesterol values
  )
...
cholesterol_summary <- cholesterol_long |>
  ...
  summarise(
    ...
    mean_chol = mean(cholesterol), # Mean cholesterol
    sd_chol = sd(cholesterol)      # Standard deviation
  )
